...
//					logger.info("step -2.1");
					List<Method> list = !methodDic.containsKey(m.getName()) ? new ArrayList<Method>() : methodDic.get(m.getName());
//					logger.info("step -2.1.2");
					list.add(m);
//					logger.info("step -2.1.3"+methodDic.containsKey(m.getName() ));
					if( !methodDic.containsKey(m.getName() ) )
						methodDic.put(m.getName(), list);
				}
			}
			catch(Exception ee)
			{
	...
	public Map<String,Object> invoke(String mName,Object[] paras)
	{
		...
		catch(Exception e)
		{
			...
			if(e instanceof InvocationTargetException){
				return ConsumerInvokerManager.createErrMsg(((InvocationTargetException) e).getTargetException().getMessage());
			}
...
//		Field f = cls.getDeclaredField("TYPE");
		...
		if( ifNumberClass(cls) && value instanceof Number )
			return true;//all is number
	...
	{
		if( value == null || !(value instanceof Number) || !ifNumberClass(cls) )
			return value;
		
		if( cls == int.class || cls == AtomicInteger.class || cls == Integer.class )
			return ((Number)value).intValue();
		if( cls == long.class || cls == AtomicLong.class || cls == Long.class )
